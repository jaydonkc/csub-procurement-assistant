...
- Production alias: `production` -> immutable version `17`
...
- Immediate rollback predecessor: version `16`
...
- API health identifies immutable Lambda version `17`; both direct Lambda URLs reject anonymous requests with `403`.
...
- The final 36-scenario raw-retrieval suite returned 34/36 exact expected-source hits, 93.1% mean procedural-term coverage, 0 internal-source leaks, 3/3 timestamp passes, and 1.837-second p95 retrieval latency.
- The final 13-scenario guided end-to-end suite through API Gateway and WAF returned 13/13 HTTP successes, 13/13 expected-source hits, 13/13 valid citation sets, 0 internal-source leaks, 0 duplicate source cards, 3/3 timestamp passes, and 5.162-second p95 latency.
- A separate 16-scenario role and behavior suite passed greetings without sources, mixed greeting/procedure routing, public vendor guidance, personalized live-lookup and action refusal, deterministic known and unknown synthetic status examples, synthetic mutation refusal, exact-threshold uncertainty, prompt injection, software clarification, linked video citations, and contextual follow-ups.
...
- Version-17 role and behavior checks passed all 16 live cases: conversational, clarification, and out-of-scope turns returned no sources; substantive procurement and contextual follow-up turns retrieved; deterministic `DEMO-*` status examples returned no citations; and personalized live lookups, transaction actions, prompt injection, and self-reported internal access remained blocked before retrieval.
- Deployed-browser checks returned a natural `hi` response with no source section and a clearly labeled synthetic chat response for `DEMO-REQ-1001` with no sources or browser errors. Cited PDF and MP4 URLs both returned HTTP `206` with `application/pdf` and `video/mp4`, respectively.
