...
- Resolved for the production agent runtime: Lambda version `22` is frozen behind the `production` alias, with Sonnet 4.6 generation, Haiku 4.5 retrieval routing and citation validation, version `21` as its immediate rollback predecessor, and version `8` retaining the first document/video source-link implementation.
...
- Resolved for guided evaluation: the July 16 post-deployment response suite passed 37/37 scenarios through the protected API, including all 14 blocker cases, with no failures and 13.603-second p95 end-to-end latency. The earlier raw-retrieval benchmark remains 34/36 exact expected-source hits with zero internal-source leakage.
